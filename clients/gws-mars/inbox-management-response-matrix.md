# Inbox Management Response Matrix — GWS & MARS

Operational response playbook for GWS and MARS inbound replies, aligned with core value propositions, outbound campaign promises, and the 3 inbox management rules.

> **Interactive Canvas:** Open [Inbox Management GWS and MARS](/cursor/stores/user/canvases/679f6bca-5488-49eb-b761-9bf525733a57/source.canvas.tsx) beside chat for scenario lookup, rule routing, and copy-ready reply templates.

---

## The 3 Inbox Management Rules

| Rule | Trigger | Action & Objective |
| :--- | :--- | :--- |
| **Rule 1: Easy to Book** | Prospect indicates interest, asks to speak, provides availability, or sends a JD. | Do **not** re-pitch. Immediately propose **2 specific slots** or provide the calendar link. |
| **Rule 2: Question Asked** | Prospect asks a question (pricing, buyers, process, candidates) or gives soft pushback. | Answer concisely (**1–2 sentences**), then bridge to a 10–15 min call using an **incentive** (peer multiples, tailored candidate summary, zero risk). |
| **Rule 3: Unknown / Complex** | Regulatory, complex specs, union rules, high-tier pricing, mandate/registration questions. | Warmly intro Founder/CEO (**Jonathon Rapacki** / **Matt Arias**) while keeping the **booking hook** active. |

---

## Client Profiles & Offer Context

### GWS Advisory Firm

| Field | Detail |
| :--- | :--- |
| **Primary Executive** | Jonathon Rapacki — Founder & Managing Director |
| **CEO Email** | `jrapacki@gwsconsulting.co` |
| **Title / Role** | Sell-Side M&A / Valuation |
| **Core Service** | Market read / M&A valuation review |
| **Workspace ID** | `6a880928da046f3b189bb4e1` |
| **Calendly** | https://calendly.com/jonathon-gws/15min |
| **Niche** | Lower middle-market sell-side M&A for privately held US companies ($5M–$50M+ revenue) across industrial, manufacturing, specialty services, and B2B |

**Core offer & Email 1 promises**
- Practical, buyer-side M&A valuation read, market readiness review, and mapping acquirer interest
- *"No assumption you're looking to sell"* / *"Not a formal appraisal"* / *"Zero commitment"*
- CTA: low-friction 10–15 minute conversation to share peer multiples and buyer rationale

**Active campaigns (6):** C1 Owner Exit Planning · C2 Growth/Momentum · C3 Industry & Market Activity · C4 Strategic Signals · C5 Strategic Value Offer · C6 Emerging / Sub-ICP

---

### Mars Staffing LLC (MARS)

| Field | Detail |
| :--- | :--- |
| **Primary Executive** | Matt Arias — Founder & Managing Director |
| **Title / Role** | Industrial & Precision Staffing |
| **Core Service** | 2–3 vetted profiles in 4–5 days |
| **Workspace ID** | `6a8d53c5977bb5537e41fcf2` |
| **Calendly** | https://calendly.com/matt-mars-staffing/15min |
| **Niche** | Skilled trade & precision manufacturing staffing (CNC, Swiss turn, tool & die, welders, maintenance, QA) across California & US facilities |

**Core offer & Email 1 promises**
- Candidate-led sourcing for trade-tested industrial and precision craft talent
- *"No long recruiting agreements or commitments up front. You review screened profiles first."*
- Concrete deliverable: 2–3 screened candidate write-ups / resumes in 4–5 days
- Pain addressed: overtime burnout, machine downtime, delivery delays, job-board turnover

**Active campaigns (4):** Generic · Enterprise & Defense · Stable & Mid-Market · High-Growth Scalers

---

## GWS Response Playbook (Sell-Side M&A Advisory)

### GWS — 1. Direct Interest / Open to Chat
- **Rule:** 1 — Easy to Book
- **Intent:** High interest / Low friction
- **Example inbound:** *"Sure, I'd be open to a quick call to see what you mean."*
- **Incentive:** Lock slot fast with 2 clear options + calendar link
- **Note:** Do not add friction or ask 5 qualifying questions before booking.

```text
Hi {{first_name}},

Glad to connect.

Do either of these windows work for a brief 10–15 minute conversation?
- Tuesday at 11:00 AM EST
- Wednesday at 2:00 PM EST

If another time suits you better, feel free to pick directly here:
https://calendly.com/jonathon-gws/15min

Looking forward,
{{sender_first_name}}
```

---

### GWS — 2. Time Window Proposed by Prospect
- **Rule:** 1 — Easy to Book
- **Intent:** Definite time proposed
- **Example inbound:** *"I can do Thursday at 2pm EST. Send an invite."*
- **Incentive:** Confirm immediately; send calendar invite with Meet link

```text
Hi {{first_name}},

Thursday at 2:00 PM EST is locked on our end.

I've sent a calendar invite with the Google Meet link over to this email.

Looking forward to speaking then,
{{sender_first_name}}
```

---

### GWS — 3. Who is the buyer / Name the acquirer?
- **Rule:** 2 — Question Asked
- **Intent:** Curious on buyer identity / vetting credibility
- **Example inbound:** *"Who is the buyer you mentioned? What is their name/fund?"*
- **Incentive:** Confidentiality prevents emailing fund names; share exact thesis + criteria live on a call

```text
Hi {{first_name}},

We work under confidentiality agreements, so we cannot broadcast specific fund names over cold email.

That said, the groups active in your space right now are well-capitalized strategic operators and private equity-backed platforms looking specifically for established operations in {{custom_sub_industry}}.

Happy to jump on a quick 10-minute call to share their exact acquisition criteria and see if their target parameters align with where {{company_name}} is headed.

Do you have 10 minutes this Wednesday or Thursday afternoon?

Best,
{{sender_first_name}}
```

---

### GWS — 4. What does this cost / What is your fee?
- **Rule:** 2 — Question Asked
- **Intent:** Price sensitivity / guarding against hidden consulting fees
- **Example inbound:** *"What do you charge for this valuation? Is there a fee?"*
- **Incentive:** 100% complimentary / no obligation; benchmark peer multiples for their files

```text
Hi {{first_name}},

There is zero cost or obligation for this initial valuation review.

We provide a practical, buyer-side view of where the market values businesses like {{company_name}} today based on recent transaction data in {{custom_sub_industry}}—not a formal paid appraisal.

If useful, we can walk you through the preliminary multiple ranges and buyer drivers on a quick 10-minute call so you have clear benchmarks for your records.

Would Thursday at 11:00 AM or 3:00 PM EST work for a brief run-through?

Best,
{{sender_first_name}}
```

---

### GWS — 5. We are NOT looking to sell right now
- **Rule:** 2 — Question Asked
- **Intent:** Defensive stance / assumption of forced sale
- **Example inbound:** *"Thanks, but we are not interested in selling. We are focused on growing."*
- **Incentive:** No pressure to sell; EV baseline + 2–3 drivers that increase exit multiples

```text
Hi {{first_name}},

Completely understand, and we are not assuming you are looking to sell today.

Most founders we speak with simply find it useful to know what their business would command in the current market, and what 2–3 drivers could increase outside valuation down the road.

If you are open to a brief 10-minute conversation, we can share the high-level multiples we are seeing across {{custom_sub_industry}} so you have that benchmark on hand for future planning.

Worth a quick chat this week, or prefer I check back next quarter?

Best,
{{sender_first_name}}
```

---

### GWS — 6. What multiples are you seeing in our space?
- **Rule:** 2 — Question Asked
- **Intent:** High-value inquiry / seeking market data
- **Example inbound:** *"What kind of multiples are buyers paying right now in our sector?"*
- **Incentive:** Give general industry range, then route to call for where their business sits in the spread

```text
Hi {{first_name}},

Across {{custom_sub_industry}}, we are currently seeing transactions settle in the 5x–8x EBITDA range for established operations, with strategic premiums paid for proprietary capabilities like {{custom_core_specialization}}.

The exact spread depends on recurring revenue concentration, margin profile, and customer diversity.

We can put together a high-level indicative range specifically for {{company_name}} and walk through the 2–3 factors that dictate the top of that band on a short 10-minute call.

Open to a brief conversation this Wednesday or Thursday?

Best,
{{sender_first_name}}
```

---

### GWS — 7. NDA Required First
- **Rule:** 2 — Question Asked
- **Intent:** Prudent founder / confidentiality gate
- **Example inbound:** *"We can't talk or share anything without an NDA in place."*
- **Incentive:** No confidential figures on call 1; mutual NDA available before proprietary disclosure

```text
Hi {{first_name}},

Completely respect that. We routinely sign mutual NDAs before any proprietary data is discussed.

That said, on a brief initial call we don't ask for any confidential figures—our goal is simply to share high-level buyer activity in {{custom_sub_industry}} and see if an M&A valuation read is even relevant for you.

If you'd like our standard mutual NDA ahead of time, let me know and I will send it right over. Otherwise, are you open to a high-level 10-minute chat this week?

Best,
{{sender_first_name}}
```

---

### GWS — 8. Timing Delay ("Check back in 3–6 months")
- **Rule:** 2 — Question Asked
- **Intent:** Soft delay / busy-season deferral
- **Example inbound:** *"We are in the middle of our busy season. Check back with us early next year."*
- **Incentive:** Offer one-page sector transaction summary for their records while pausing outreach

```text
Hi {{first_name}},

Understood. I will pause outreach and touch base with you in early {{next_quarter_month}}.

If you'd like me to email over a one-page summary of recent M&A transactions in {{custom_sub_industry}} in the meantime for your records, just let me know.

Wishing you a productive busy season.

Best,
{{sender_first_name}}
```

---

### GWS — 9. Complex Technical / Regulatory / Mandate Questions
- **Rule:** 3 — CEO Hand-off
- **Intent:** Sophisticated / Institutional / Beyond VA knowledge
- **Example inbound:** *"What is your specific mandate fee structure, are you FINRA/SIPC registered, and what deals have you closed in aerospace precision under $40M?"*
- **Incentive:** Introduce Jonathon Rapacki (Founder & Managing Director); keep Calendly live
- **Note:** CC: Jonathon Rapacki

```text
Hi {{first_name}},

Thank you for the detailed questions.

I've CC'd Jonathon Rapacki, our Founder and Managing Director at GWS Advisory, who leads our advisory mandates and transaction execution in this sector.

Jonathon will follow up directly with the specific transaction background and regulatory structure you requested.

In the meantime, so we can align schedules without back-and-forth, feel free to grab a convenient 15-minute slot on Jonathon's calendar here:
https://calendly.com/jonathon-gws/15min

Best regards,
{{sender_first_name}}
GWS Advisory Firm
```

---

### GWS — 10. Specific Buyer Mandate Verification
- **Rule:** 3 — CEO Hand-off
- **Intent:** High-value qualified opportunity requiring principal discretion
- **Example inbound:** *"We have $15M revenue and 20% EBITDA in defense machining. If your buyer is serious, what are their minimum revenue and geographical parameters?"*
- **Incentive:** Loop Jonathon for buyer criteria; retain booking link
- **Note:** CC: Jonathon Rapacki

```text
Hi {{first_name}},

Those parameters fit directly into the core criteria of the buyer groups we are speaking with.

I've looped in Jonathon Rapacki, our CEO & Managing Director, who is actively managing those buyer relationships and can speak directly to their geographic and size criteria.

Jonathon, see {{first_name}}'s profile above regarding {{company_name}}.

{{first_name}}, you can also reserve a time directly on Jonathon's calendar below for a confidential conversation:
https://calendly.com/jonathon-gws/15min

Best,
{{sender_first_name}}
GWS Advisory Firm
```

---

## MARS Response Playbook (Skilled Trade & Manufacturing Staffing)

### MARS — 1. Direct Interest / Open to Call
- **Rule:** 1 — Easy to Book
- **Intent:** Urgent or active floor hiring need
- **Example inbound:** *"Yes, we could use some help on the floor. Let's do a quick call."*
- **Incentive:** Lock slot fast with 2 clear options + calendar link

```text
Hi {{first_name}},

Glad to connect.

Do either of these windows work for a brief 10-minute call to discuss what your floor needs?
- Tomorrow (Tuesday) at 1:30 PM PST
- Wednesday at 10:00 AM PST

Or feel free to grab a time that fits your floor schedule directly here:
https://calendly.com/matt-mars-staffing/15min

Best,
{{sender_first_name}}
```

---

### MARS — 2. Prospect Sends Job Description Directly
- **Rule:** 1 — Easy to Book + Delivery Commitment
- **Intent:** Immediate active requisition
- **Example inbound:** *"Here is the JD for our CNC Lathe setup operator. Let's see what you have."*
- **Incentive:** Confirm receipt; commit 2–3 profiles in 3–4 business days; short calibration call

```text
Hi {{first_name}},

Received, thank you. Reviewing the specifications now.

We are pulling 2–3 screened profiles from our pipeline who have verified experience on these setups and hold tight tolerance without a long ramp. I will have those candidate write-ups in your inbox within 3–4 business days.

To make sure we only send candidates that match your exact shift, wage, and machine requirements, would you have 5–10 minutes tomorrow afternoon for a quick calibration call?

You can lock in a quick time here:
https://calendly.com/matt-mars-staffing/15min

Best,
{{sender_first_name}}
```

---

### MARS — 3. What are your fees / markup / terms?
- **Rule:** 2 — Question Asked
- **Intent:** Commercial clarity / avoiding agency lock-in
- **Example inbound:** *"What are your placement fees and contract terms? We don't sign exclusive agreements."*
- **Incentive:** $0 upfront, contingent only, review profiles free; exact rate card on a quick call

```text
Hi {{first_name}},

We work on a straightforward performance model: zero upfront retainer, no exclusivity required, and no cost to review screened candidate summaries. You only pay if you interview and decide to bring someone onto your floor.

Our standard fee is a competitive flat percentage upon hire, backed by a 60-day replacement guarantee.

If you have an active seat or want to see the exact rate card for {{custom_talent_type}}, happy to jump on a quick 10-minute call to share the details and see what profiles we have available in your area.

Do you have a few minutes this Wednesday or Thursday?

Best,
{{sender_first_name}}
```

---

### MARS — 4. Do you have candidates in [City / Specific Machine]?
- **Rule:** 2 — Question Asked
- **Intent:** Verifying local talent pool depth
- **Example inbound:** *"Do you have 5-axis Haas CNC machinists in Ontario, CA right now?"*
- **Incentive:** Confirm regional footprint; offer 2 tailored candidate write-ups vs generic resumes

```text
Hi {{first_name}},

Yes, Southern California precision machining is right in our core footprint. We maintain active files on trade-tested CNC operators and setup machinists across the Inland Empire and Greater LA who have proven backgrounds on Haas, Mazak, and Doosan controls.

Rather than sending general resumes, I can pull 2 relevant candidate write-ups that match your exact machine setup and tolerance requirements.

Would you have 10 minutes tomorrow or Wednesday to review what your floor needs so I only send exact matches?

You can also pick a quick time here:
https://calendly.com/matt-mars-staffing/15min

Best,
{{sender_first_name}}
```

---

### MARS — 5. We are fully staffed right now
- **Rule:** 2 — Question Asked
- **Intent:** No active search today / potential future client
- **Example inbound:** *"Thanks, but our shop is fully staffed at the moment. No openings."*
- **Incentive:** Preventative backup talent on file at $0 so they never scramble on overtime

```text
Hi {{first_name}},

Glad to hear the floor is in good shape—that is rare in manufacturing right now.

Most plant managers we work with simply keep 1–2 screened profiles on file with us as preventative backup so that when an unexpected two-week notice hits, they don't lose machine capacity scrambling to hire from scratch.

If you'd like to have a vetted backup profile on deck for your hardest-to-fill trade, reply with the role and I'll send one summary for your records—no strings attached.

Otherwise, happy to check back with you in a couple of months.

Best,
{{sender_first_name}}
```

---

### MARS — 6. We have an in-house HR / Recruiting team
- **Rule:** 2 — Question Asked
- **Intent:** Internal gatekeeper / standard objection
- **Example inbound:** *"All our hiring is handled internally by our HR department."*
- **Incentive:** We don't replace HR; we supply passive employed craftspeople when seats sit open past 30 days

```text
Hi {{first_name}},

Completely understand. We don't replace internal HR—we act as an external specialist pipeline specifically for trade-tested shop floor talent that doesn't apply to job boards.

Most internal teams lean on us when a specialized machine seat sits open past 30 days and overtime starts wearing on the crew.

If you'd be open to seeing a couple of anonymized candidate profiles for your hardest-to-fill trade to keep in your back pocket, worth a quick 10-minute chat?

Best,
{{sender_first_name}}
```

---

### MARS — 7. Referral / Routing to Operations / Plant Manager
- **Rule:** 1 — Direct Routing
- **Intent:** Wrong contact; needs plant/ops owner
- **Example inbound:** *"I don't handle this directly. Talk to our Plant Manager, Dave (dave@company.com)."*
- **Note:** Then immediately initiate outreach to the referred contact, referencing the referral.

```text
Hi {{first_name}},

Appreciate the direction. I will reach out to Dave directly and reference our note.

Best,
{{sender_first_name}}
```

---

### MARS — 8. Complex Technical Specs / Union Rules / Multi-facility Contract
- **Rule:** 3 — CEO Hand-off
- **Intent:** High compliance / specialized enterprise requirements
- **Example inbound:** *"We are an ITAR-registered defense contractor with union floor rules and specific DOD security clearance requirements. Can your contract accommodate this?"*
- **Incentive:** Introduce Matt Arias to review compliance framework
- **Note:** CC: Matt Arias. High-value enterprise lead requiring founder sign-off.

```text
Hi {{first_name}},

Thank you for raising that. Precision defense and regulated production environments are an area we handle carefully, including US citizenship verification and compliance flow-downs.

I've looped in Matt Arias, Founder and Managing Director of Mars Staffing, who manages our enterprise client agreements and defense facility compliance.

Matt can walk you through our specific vetting protocols and contract terms for union/regulated facilities.

In the meantime, feel free to grab a brief 10–15 minute window on Matt's calendar so we can address your floor requirements directly:
https://calendly.com/matt-mars-staffing/15min

Best regards,
{{sender_first_name}}
Mars Staffing LLC
```

---

### MARS — 9. Custom Bill Rates / Volume Discount Inquiry
- **Rule:** 3 — CEO Hand-off
- **Intent:** Large enterprise deal / custom commercial terms needed
- **Example inbound:** *"We are adding a 2nd shift with 15 CNC operators and 3 QA leads. What kind of tiered volume pricing or temp-to-perm rate can you offer?"*
- **Incentive:** CC Matt for volume enterprise partnership structure; keep Calendly live
- **Note:** CC: Matt Arias

```text
Hi {{first_name}},

That is a significant expansion, and we can definitely structure custom tiered volume rates for a 12+ headcount ramp.

I've CC'd Matt Arias, our Founder and Managing Director, who structures our volume enterprise partnerships and oversees talent allocation.

Matt will review the volume tiering and follow up with a tailored commercial structure.

To review the timeline and launch requirements without delay, feel free to lock in 15 minutes with Matt directly here:
https://calendly.com/matt-mars-staffing/15min

Best,
{{sender_first_name}}
Mars Staffing LLC
```

---

## Inbox Manager Quality Checklist

1. **Confirm Client & Campaign:** Never mix M&A language into Staffing threads or vice versa.
2. **Rule 1 (Easy Yes):** Lock calendar with 2 times or link. Never add qualification gates to someone ready to meet.
3. **Rule 2 (Questions):** Give a 1-sentence answer, then offer an incentive (peer benchmark / profile teaser) to book.
4. **Rule 3 (CEO Hand-off):** CC Jonathon Rapacki (GWS) or Matt Arias (MARS). Introduce as CEO/Founder and retain the booking link.

### Universal Tone & Execution

1. **Never Mix Niche Dictionaries**
   - GWS: targets, mandates, buyer interest, peer multiples — never "candidates / placements / resumes"
   - MARS: craft talent, workstations, overtime, tolerances — never "acquisitions / valuations"
2. **Never Add Friction to an Easy Yes** — if they say yes or suggest a day, lock it in immediately.
3. **Always Anchor Rule 2 with a Clear Incentive** — answer alone = ping-pong; answer + incentive = calls.
4. **Always Retain the Booking Hook on Rule 3** — never just say "I'll let them reply"; always include the founder's calendar link.

---

## Quick Reference — Calendars & CCs

| Client | Founder | CC Email / Role | Calendar |
| :--- | :--- | :--- | :--- |
| GWS | Jonathon Rapacki | Founder & Managing Director | https://calendly.com/jonathon-gws/15min |
| MARS | Matt Arias | Founder & Managing Director | https://calendly.com/matt-mars-staffing/15min |
